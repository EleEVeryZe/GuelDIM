...
export class GoogleDriveInvestmentRepository implements InvestmentRepository {
  async getAll(fileId: string): Promise<Investment[]> {
    ...
    try {
      return JSON.parse(response.body) as Investment[];
    } catch (error) {
  ...
  async add(fileId: string, investments: Investment[]): Promise<void> {
    ...
    const merged = [...existing, ...investments];
  ...
  async remove(fileId: string, investmentId: string): Promise<void> {
    const existing = await this.getAll(fileId);
    const filtered = existing.filter((i) => i.id !== investmentId);
